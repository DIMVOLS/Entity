...
Features=[x*0.35+a*0.45+y*0.2+c*0.22+b*0.33 for x,a,y,c,b in 
...
confidence_score=[(x+y+z)/3 for x,y,z in zip(spec_symbols,tags,f_threshold)]

#norm_Features=[(x/max(Features))for x in Features]
norm_Features=[(x-min(Features))/(max(Features)-min(Features)) if y > 0.5 
